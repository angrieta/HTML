...
let now = new Date();
let mon = now.getMonth()+1; //자바스크립는 0부터 인식하기 때문에, getMonth로 월 출력 시 +1을 반드시 작성해야한다.
let day = now.getDate();
...
//메서드와 속성을 작성할때는 반드시 앞에 객체(object)선언이 되어야 한다.
//객체.메서드();
//객체.속성;
//============================배열과 객체
//배열 이전 형태 기본 변수
...
console.log(`box1의 값은 ${box2*1}이다`); // box1의 값은 20이다.
...
//배열 생성과 호출 연습
let colorSet = ['흰색','검정색','빨강색','노랑색','파랑색'];
console.log(colorSet[0], colorSet[2], colorSet[3]); //랜덤하게 3개 고르기
...
//객체.shift() 배열에 시작 위치에서 값 1개 제거
let numbers = [5,6,7,8];
console.log(numbers.length); //4
console.log(numbers[3]); //8
numbers.push(9); //배열에 끝에 9 숫자데이터 삽입
console.log(numbers);
numbers.pop(); //배열의 끝에 값 1개 제거
console.log(numbers);
numbers.unshift(4); // 배열 시작위치에 4추가
console.log(numbers);
numbers.shift(); // 배열 시작위치에 4제거
console.log(numbers);
numbers.push(9);
numbers.push(10);
//메가박스 빠른 예매 시스템 이해
...
//3. 추가된 인원이 희망자석을 선택하면 해당 좌석정보가 위 2번 저장소에 대입된다.
let megabox = [];
...
megabox.shift();
console.log(megabox);
console.log(`최종결제 금액은 ${megabox.length*megaboxPrice}원 입니다.`);
//커피 제공 메뉴와 장바구니
console.log('=====================')
